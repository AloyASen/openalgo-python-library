### layr0_imc Indicators Performance Audit (NumPy/Numba) — Audit 02

This follow-up audit reflects recent refactors and optimizations. It validates progress and lists the remaining high-value improvements. References use module paths like `layr0_imc/indicators/*` and utilities in `layr0_imc/indicators/utils.py`.

### What’s improved since Audit 01
- **JIT shim available**: `layr0_imc/numba_shim.py` provides cached, fastmath-enabled `jit/njit/prange` defaults.
...
- **Adopt the JIT shim everywhere**
  - Many modules still use `from numba import jit`. Switch to `from layr0_imc.numba_shim import jit` (and `njit/prange`) across `trend.py`, `oscillators.py`, `volatility.py`, etc., to standardize `fastmath=True, cache=True`.
...
- Across all: switch `numba.jit` to `layr0_imc.numba_shim.jit` and remove duplicated EMA/ATR kernels.
